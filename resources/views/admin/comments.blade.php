...
@section('action')
    @if($unverified_count>0)
        <button class="btn btn-sm btn-outline-danger swal-dialog-target"
                data-dialog-msg="delete {{ $unverified_count }} 条未审核评论？"
                data-url="{{ route('comment.delete-un-verified', ['ids'=>$unverified_ids]) }}"
                data-method="delete">Delete unreviewed
        </button>
    @endif
@endsection
@if($comments->isEmpty())
                ...
                <td>
                    @if($comment->trashed())
                        <button type="submit"
                                ...
                                data-dialog-msg="Delete this comment permanently？"
                    ...
                    @else
                        <button type="submit"
                                class="btn btn-danger swal-dialog-target"
                                data-dialog-msg="Are you sure you want to delete this comment?"
                                data-toggle="tooltip"
