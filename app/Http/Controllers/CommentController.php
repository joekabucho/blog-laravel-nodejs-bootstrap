...
class CommentController extends Controller
{
    ...
    public function update(Request $request, $comment_id)
    {
        ...
        if ($this->commentRepository->update($request->get('content'), $comment)) {
            ...
            if ($redirect)
                return redirect($redirect)->with('success', 'Successfully modified');
            return back()->with('success', 'Successfully modified');
        }
        return back()->withErrors('fail to edit');
    }
...
//        dd($request->all());
    ...
    public function destroy($comment_id)
    {
        ...
        if (request()->expectsJson()) {
            return response()->json(['status' => 500, 'msg' => 'failed to delete']);
        }
